...
void emulation_start() {
  ...
  xTaskCreatePinnedToCore(emulation_task, "emulation task", 4096, NULL, 5, &emulationTaskHandle, ARDUINO_RUNNING_CORE == 0 ? 0 : 1);
}
...
void emulation_task(void *p) {
  for(;;) {
    ...
    if(currentMachine->game_started)   
      ...
      vTaskDelay(pdMS_TO_TICKS(2)); // give a millisecond delay to make the watchdog happy
  }
}
